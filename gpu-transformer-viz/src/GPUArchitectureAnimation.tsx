...
// ===================== Detail Panel =====================
function DetailPanel({ selectedKey }: { selectedKey: string }) {
  return (
    <motion.div
      key={selectedKey}
...
function SchedulerDemo() {
  ...
  const stalled = (w: number, t: number) => ((w * 17 + (t % 13) * 7) % 100) < stallPct; // ~stallPct%
  const activeWarpFor = (sched: number, t: number) => {
