"""
Training callbacks for RL trading agents.

This module contains callbacks for monitoring and controlling training progress.
"""
...
class TrainingProgressCallback(BaseCallback):
    ...
    def __init__(
        ...
        # Episode profitability tracking
    ...
    def _on_step(self) -> bool:
        ...
        if len(actions) > 0:
            action = int(actions[0]) if isinstance(actions[0], (np.ndarray, np.generic)) else int(actions[0])
        ...
        if dones[0]:
            ...
            # Track profitability (positive reward = profitable episode)
    ...
    def get_training_stats(self) -> Dict:
        ...
        # Calculate win rate
        win_rate = (self.profitable_episodes / self.total_completed_episodes * 100
                   if self.total_completed_episodes > 0 else 0)

        # Get final and best episode rewards
        final_episode_reward = self.episode_rewards[-1] if len(self.episode_rewards) > 0 else 0
        best_episode_reward = max(self.episode_rewards) if len(self.episode_rewards) > 0 else 0
