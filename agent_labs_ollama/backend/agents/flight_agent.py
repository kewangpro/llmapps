...
class FlightAgent(BaseAgent):
    ...
    def execute(self, query: str) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"✈️  FlightAgent analyzing: '{query}'")
...
- departure_date: Departure date in YYYY-MM-DD format (if year not mentioned, use 2024)
...
- "flights from SFO to Tokyo departing December 20, 2024" → {{"origin": "SFO", "destination": "Tokyo", "departure_date": "2024-12-20"}}
- "round trip from LAX to Paris, leaving Jan 10 2025 returning Jan 20 2025" → {{"origin": "LAX", "destination": "Paris", "departure_date": "2025-01-10", "return_date": "2025-01-20"}}
- "search flights San Francisco to New York on Christmas 2024" → {{"origin": "San Francisco", "destination": "New York", "departure_date": "2024-12-25"}}
- "one way ticket Miami to Seattle March 15" → {{"origin": "Miami", "destination": "Seattle", "departure_date": "2024-03-15"}}

IMPORTANT:
1. Always convert dates to YYYY-MM-DD format
2. If year is not mentioned, assume 2024
3. For month names (January, Feb, etc.), convert to numeric format
4. Only include return_date if explicitly mentioned or if "round trip" is indicated
5. Extract city names or airport codes exactly as mentioned
...
{{"origin": "city or code", "destination": "city or code", "departure_date": "YYYY-MM-DD", "return_date": "YYYY-MM-DD or null"}}"""
            ...
            tool_result = self._execute_tool_script("flight", params)
            ...
            logger.error(f"✈️  FlightAgent error: {str(e)}")
            return {
                "tool": "flight",
                "success": False,
                "error": str(e),
